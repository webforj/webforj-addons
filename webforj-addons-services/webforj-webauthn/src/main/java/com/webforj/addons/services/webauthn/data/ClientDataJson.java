...
/**
 * This class represents the client data passed to {@code navigator.credentials.create()} or {@code
 * navigator.credentials.get()}. It contains information such as the cryptographic challenge,
...
public class ClientDataJson {
  ...
  /**
   * Indicates whether the calling context is an {@code <iframe>} that is not same origin with its
   * ancestor frames.
   */
  ...
  /**
   * Parses the client data JSON from a base64url-encoded string and constructs a {@code
  ...
  /**
   * Checks whether the calling context is an {@code <iframe>} that is not same origin with its
   * ancestor frames.
   *
