 ...
 *
 * @see <a href="https://www.w3.org/TR/webauthn-3/#dictdef-publickeycredentialcreationoptions">§5.4.
 *      Options for Credential Creation (dictionary PublicKeyCredentialGetOptions)</a>
 */
public class PublicKeyCredentialGetOptions {

  /**
   * A challenge provided by the relying party, which the selected authenticator signs along with
   * other data when generating an authentication assertion. This helps prevent replay attacks and
   * ensures the authenticity of the request.
   *
   * @see <a href="https://www.w3.org/TR/webauthn-3/#sctn-cryptographic-challenges">§13.1
   *      Cryptographic Challenges</a>
   */
  private final String challenge;

  /**
   * Specifies the time, in milliseconds, that the caller is willing to wait for the operation to
   * complete. This is treated as a hint and may be overridden by the client.
   */
  private long timeout = 300000L;

  /**
   * Specifies the identifier claimed by the relying party. If omitted, the value will be set by the
   * client.
   */
  private String rpId;
   ...
   * acceptable to the caller. The list is ordered by the caller's preference, with the most
   * preferred credential first.
   */
   ...
   * {@code navigator.credentials.get()} operation. Eligible authenticators are filtered based on
   * this requirement. If not set, the default in the browser is
   * {@link UserVerificationRequirement#PREFERRED}.
   *
   * @see UserVerificationRequirement
   * @see <a href="https://www.w3.org/TR/webauthn-3/#enum-userVerificationRequirement">§5.8.6. User
   ...
   *
   * <p>
   * For example, if transaction confirmation is sought from the user, then the prompt string might
  ...
  /**
   * Converts this PublicKeyCredentialGetOptions instance to its JSON representation.
   *
  ...
  /**
   * Retrieves the additional parameters requesting additional processing by the client and
   * authenticator.
   *
  ...
  /**
   * Sets the additional parameters requesting additional processing by the client and
   * authenticator.
   *
