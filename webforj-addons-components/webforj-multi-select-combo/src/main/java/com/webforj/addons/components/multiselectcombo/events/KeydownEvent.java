...
@EventOptions(
    data = {
      @EventOptions.EventData(key = "altKey", exp = "event.detail.altKey"),
...
public class KeydownEvent extends ComponentEvent<MultiSelectCombo> {
  ...
  /**
   * Checks if the Meta key was pressed during the keydown event (often referred to as Command key
   * on macOS).
   *
  ...
  /**
   * Gets the key code of the pressed key according to the DOM Level 3 KeyboardEvent standard.
   *
   * @see <a href= "https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/code">MDN
