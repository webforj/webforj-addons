 ...
 * structure.
 *
 * <p>The {@code PropertiesPanel} component is offers functionality for handling various types of
...
public class PropertiesPanel extends ElementComposite
    implements HasClassName<PropertiesPanel>,
  ...
  /**
   * Property that represents the indices of the expanded groups in the properties panel. This
   * property must be provided as a list of integers, where each integer corresponds to the index of
   * a group in the {@link #schemaProp} that is currently expanded.
   */
  private final PropertyDescriptor<List<Integer>> expandedGroupIndicesProp =
      PropertyDescriptor.property("expandedGroupIndices", new ArrayList<>());
  ...
  public PendingResult<HashMap<String, Object>> getProperties() {
    return this.getElement()
        .callJsFunctionAsync("getProperties")
        .thenApply(
            propertiesJson -> {
  ...
  /**
   * Updates the value of a property based on its name and applies the change to the corresponding
   * UI element.
   *
   * <p>This method ensures that the provided value is assigned to the specified property,
   * triggering any necessary updates in the UI. The expected value type depends on the schema
   * definition of the property:
   *
   * <ul>
   *   <li><b>String schema type:</b> Accepts values of type {@code String}, {@code Number}, or
   *       {@code Boolean}
   *   <li><b>Number schema type:</b> Accepts values of type {@code String} or {@code Number}
   *   <li><b>Boolean schema type:</b> Accepts values of type {@code Boolean}.
   *   <li><b>Enum schema type:</b> Accepts values of type {@code Number} (representing an index in
   *       an enumeration).
   * </ul>
   *
   * If the provided value does not match the expected type for the property, the client-side
   * implementation rejects the update and log an error.
   *
   * @param name The name of the property to update.
   * @param value The new value to assign to the property. The type must adhere to the expected
   *     schema constraints.
   * @return A {@link PendingResult} that completes when the property update has been processed.
   */
  public PendingResult<Void> setSchemaProperty(String name, Object value) {
    return this.getElement()
        .callJsFunctionAsync("setProperty", name, value)
  ...
  public PendingResult<Void> expandByIndex(int index) {
    return this.getElement().callJsFunctionAsync("expandByIndex", index).thenApply(result -> null);
  }
   ...
   * @return A {@link PendingResult} that completes when the collapse action is applied.
   */
  public PendingResult<Void> collapseByIndex(int index) {
    return this.getElement()
        .callJsFunctionAsync("collapseByIndex", index)
        .thenApply(result -> null);
  }
   ...
   *
   * @return A {@link PendingResult} that completes when all headers have been expanded.
   */
   ...
   *
   * @return A {@link PendingResult} that completes when all headers have been collapsed.
   */
  ...
  /**
   * Sets the indices of the expanded groups. Ensures that only valid indices (non-negative and
   * within the bounds of the schema's length) are stored. Duplicate indices are removed to maintain
   * uniqueness.
   *
